...
import static com.raiyan.recyclerinstantplay.Instant_Utils.getString;

public class Instant_CustomRecyclerView extends RecyclerView {
    ...
    public Instant_CustomRecyclerView(@NonNull Context context) {
        super(context);
    }

    public Instant_CustomRecyclerView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public Instant_CustomRecyclerView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle) {
...
//                        Log.d("pos", "recyclerview left: " + rect_parent.left + " | right: " + rect_parent.right + " | top: " + rect_parent.top + " | bottom: " + rect_parent.bottom);
                if (playOnlyFirstVideo) {
                    ...
                    for (int i = firstVisiblePosition; i <= lastVisiblePosition; i++) {
                        ...
                        try {
                            Instant_CustomViewHolder cvh = (Instant_CustomViewHolder) holder;
...
//                                    Log.d("trace", i + " foundFirstVideo: " + cvh.getVideoUrl());
                                    ...
                                    if (getString(_act, cvh.getVideoUrl()) != null && new File(getString(_act, cvh.getVideoUrl())).exists()) {
                                        ((Instant_CustomViewHolder) holder).initVideoView(getString(_act, cvh.getVideoUrl()), _act);
                                    } else {
                                        ((Instant_CustomViewHolder) holder).initVideoView(cvh.getVideoUrl(), _act);
                                    }
                                    Runnable myRunnable = new Runnable() {
                                        @Override
                                        public void run() {
                                            if (!((Instant_CustomViewHolder) holder).isPaused())
                                                ((Instant_CustomViewHolder) holder).playVideo();
                                        }
                                    };
...
//                                    Log.d("trace", i + " not foundFirstVideo: ");
                                    ((Instant_CustomViewHolder) holder).pauseVideo();
                                }
                            }
                        } catch (Exception e) {
                        }
...
//                                Log.d("trace", i + " contains: " + rect_parent.contains(rect_child));
    ...
    public void preDownload(List<String> urls) {
        if (!Instant_Utils.isConnected(_act)) return;
        HashSet<String> hashSet = new HashSet<String>();
        hashSet.addAll(urls);
        urls.clear();
        urls.addAll(hashSet);
        for (int i = 0; i < urls.size(); i++) {
            if ((Instant_Utils.getString(_act, urls.get(i)) == null || !(new File(getString(_act, urls.get(i))).exists())) && urls.get(i) != null && !urls.get(i).equalsIgnoreCase("null")) {
                Intent intent = new Intent(Intent.ACTION_SYNC, null, _act, Instant_DownloadService.class);
    ...
    public void stopVideos() {
        for (int i = 0; i < getChildCount(); i++) {
            if (findViewHolderForAdapterPosition(i) instanceof Instant_CustomViewHolder) {
                final Instant_CustomViewHolder cvh = (Instant_CustomViewHolder) findViewHolderForAdapterPosition(i);
